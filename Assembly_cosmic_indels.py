...
Arguments:
    ...
    -in | --indel 					  (optional argument, default = 'NO')
...
def main():
    ...
    try:
        myopts, args = getopt.getopt(sys.argv[1:],"w:sq:snp:in:t:i:o:c",["workdir=","sqlitedb=","snp=","indel=",'tmp_folder=','tis_id=','sqlite_out=','cosmic='])
    ...
    print(" Nunmber of COSMIC INDELs in transcript     " + str(cosmic))
    print("")
    ...
    for id in tis_id_list:
        ...
        construct_translation_product(species,assembly,chrs,ensembldb,workdir,sqlitedb,snp,indel,analysis_ID,nr_of_cores,cosmic)
    ...
    try:
        ...
        with con:
            ...
            if snp=="NO":
                ...
                if indel=="NO":
                    tableName = "TIS_"+str(analysis_ID)+"_snp"+snp+"_transcripts"
                else:
                    tableName ="TIS_"+str(analysis_ID)+"_snp"+snp+"_indel"+ indel +"_transcripts"
...
def construct_translation_product(species,assembly,chrs,ensembldb,workdir,sqlitedb,snp,indel,analysis_ID,nr_of_cores,cosmic):
    '''
    :param species:
    :param assembly:
    :param chrs:
    ...
    pool = multiprocessing.Pool(processes=nr_of_cores)
    [pool.apply_async(assembly_per_chr, args=(species,assembly,chrs,ensembldb,workdir,sqlitedb,snp,indel,analysis_ID,chr,cosmic)) for chr in chrs]
...
def assembly_per_chr(species,assembly,chrs,ensembldb,workdir,sqlitedb,snp,indel,analysis_ID,chr,cosmic):
    #for chr in chrs:
    print (chr)
    sys.stdout.flush()
    ...
    for key in transcript_per_chromosome:
        ...
        sort_all_INDELS_and_SNPs_for_this_sequence1={}
        TIS = 0
        ref_seq = ''
        ...
        for rank in exons_for_transcript:
            ...
            for genomeposition in all_INDELS_and_SNPs_for_this_sequence:
                ...
                sort_all_INDELS_and_SNPs_for_this_sequence1[sequence_position] = variant_info
            sort_all_INDELS_and_SNPs_for_this_sequence1 = OrderedDict(sorted(sort_all_INDELS_and_SNPs_for_this_sequence1.items()))

        #print('sort all INDELS and SNPs for this sequence',sort_all_INDELS_and_SNPs_for_this_sequence)
        #print('cosmic',cosmic)
       # print('number_indels',number_indels)
        #print(sort_all_INDELS_and_SNPs_for_this_sequence1)
        
        for position in sort_all_INDELS_and_SNPs_for_this_sequence1:
            
                               
            if number_indels < int(cosmic):
                    
                variant_info = sort_all_INDELS_and_SNPs_for_this_sequence1[position]
                
                sort_all_INDELS_and_SNPs_for_this_sequence[position] = variant_info
                number_indels+=1
                #print('n',number_indels)
                #print('c',cosmic)

            else:
                pass
        sort_all_INDELS_and_SNPs_for_this_sequence = OrderedDict(sorted(sort_all_INDELS_and_SNPs_for_this_sequence.items()))
        #if len(sort_all_INDELS_and_SNPs_for_this_sequence)>0:
        #print('all INDELS2', sort_all_INDELS_and_SNPs_for_this_sequence1,'INDELs max cosmic2', sort_all_INDELS_and_SNPs_for_this_sequence,len(sort_all_INDELS_and_SNPs_for_this_sequence))
        sys.stdout.flush()
