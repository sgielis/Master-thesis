...
while (<IN>){
    ...
    my $refString = $columns[2];
    
    if (!$altString){ # skip the line if the alternative base is undefined
        next;
    }
     if (!$refString){ # skip the line if the alternative base is undefined
        next;
    }
...
while (length($refString) > 0){
        
        if ($refString =~ /^(A+)/){
            $refNumber .= "1"x(length($1));
            
            $refString =~ s/^$1//;
        } elsif ($refString =~ /^(T+)/){
            $refNumber .= "2"x(length($1));
            
            $refString =~ s/^$1//;
        } elsif ($refString =~ /^(C+)/){
            $refNumber .= "3"x(length($1));
            
            $refString =~ s/^$1//;
        } elsif ($refString =~ /^(G+)/){
            $refNumber .= "4"x(length($1));
            
            $refString =~ s/^$1//;
        } elsif ($refString =~ /^([^ATCG]+)/) {
            $refNumber .= "0"x(length($1));
    ...
    my $index = $chr."0"x$numLeft.$pos."A".$altNumber."R".$refNumber;
    print OUT $index;
